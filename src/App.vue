  ...
  export default {
    setup() {
      ...
      const fetchAllPokemons = async () => {
        try {
          const promises = Array.from({ length: 151 }, (_, i) => axios.get(`https://pokeapi.co/api/v2/pokemon/${i + 1}`));
      ...
      const startBattle = () => {
        ...
        rounds.value = Math.min(rounds.value, 10); 
      ...
      const getDropdownLabel = (statsSelection, player) => {
        return statsSelection.length > 0 ? `Estadísticas seleccionadas: ${statsSelection.join(', ')}` : `${player} - Seleccionar Estadísticas`;
      };
...
.torneofinish {
  ...
  #app {
    background-image: url('https://i.pinimg.com/originals/a4/2f/fe/a42ffee630841b57cdbaf283bbce7bb5.gif'); 
